...
export const END_POINTS = {
  ...
  UPDATE_PARENT: (parentId: string) => `/auth/users/edit/${parentId}`, // PARENT 엔드 포인트로 PATCH메소드 사용하면 될 것 같은데 굳이 나눈 이유를 모르겠음
  DELETE_PARENT: (parentId: string) => `/auth/users/delete/${parentId}`, // PARENT 엔드 포인트로 DELETE메소드 사용하면 될 것 같은데 굳이 나눈 이유를 모르겠음
  PARENT: (parentId: string) => `/auth/users/${parentId}`,
  NOTE: (noteId: string) => `/note?id=${noteId}`,
  ...
  FIND_FRIEND_LIST: (playgroundId: string) => `/find-friend/${playgroundId}`,
